...
Teacher access is a single shared password, read from the `TEACHER_PASSWORD` environment variable. There is no user registration or OAuth flow.

| Procedure | Type | Input | Description |
|-----------|------|-------|-------------|
| `teacher.login` | Mutation | `{ password: string }` | Validates the password and returns a signed session token |
| `teacher.verify` | Query | none | Reports whether the request carries a valid session |

Signing in returns a **signed, expiring token** rather than the password itself. The token is `<expiryMs>.<HMAC-SHA256 signature>`, signed with the password as the key, and lasts **12 hours**. Because it is signed rather than stored, it survives the free tier's frequent restarts, and rotating the password immediately invalidates every existing session.

The token travels in an **`Authorization: Bearer` header**, never in the procedure input. This matters because tRPC serialises query input into the URL: an earlier version passed the token as input, which wrote the teacher's password into the query string of every dashboard poll — and therefore into every HTTP access log, every 10 seconds.

Teacher-only procedures use `teacherProcedure`, which rejects any request without a valid session before the handler runs. The logic lives in `server/_core/teacherAuth.ts` and is covered by `server/teacherAuth.test.ts`.
...
| `TEACHER_PASSWORD` | Yes | The password used to access the teacher dashboard. The server **refuses to start in production** without it |
...
Go to the Render dashboard, open the `surviving-to-thriving` service, click **Environment**, update `TEACHER_PASSWORD`, and save. Render redeploys automatically and the change takes effect in 2–3 minutes.

Because session tokens are signed with the password, changing it **signs out every existing session** — which is what you want when rotating a credential you believe is exposed.

If `TEACHER_PASSWORD` is missing, the server **refuses to start in production** and exits non-zero rather than falling back to a default. Render marks that deploy failed and keeps the previous version serving, so a typo takes the deploy down rather than leaving the dashboard open with a publicly known password.
...
**Teacher access.** A single shared password, stored as an environment variable and never sent to the client. Signing in returns a signed 12-hour token (see 5.1), so the credential held in `localStorage` is a bounded-lifetime token rather than the password. Password comparison is constant-time. Production refuses to start without `TEACHER_PASSWORD` rather than using a fallback.

**Credentials in URLs.** Teacher tokens are sent as an `Authorization` header, so they do not appear in URLs, browser history, or HTTP access logs.

**Student submissions** are public endpoints — anyone with the session link can submit, by design, since students should not need accounts. Session codes are random 8-character `nanoid` strings, impractical to guess.

**Data.** IP addresses are recorded with each submission but never displayed. The database connection uses SSL as required by TiDB Cloud, and all browser traffic is HTTPS via Render's managed certificates.

**Known limitations.** The shared password means there is no per-teacher identity or audit trail, which is appropriate for a single-instructor tool but would need rethinking for multiple instructors. There is no rate limiting on `teacher.login`.
